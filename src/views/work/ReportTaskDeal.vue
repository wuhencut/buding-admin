<template>
	<div class="page">
		<div class="page-title">举报任务处理</div>
		<el-table ref="multipleTable" :data="list">
			...
			<el-table-column prop="content" label="提交信息" width="200">
				<template slot-scope="scope">
					...
					<!--</el-popover>-->
					<el-button type="text" @click="showAppealContent(scope.row)">举报：【{{scope.row.task_title}}】</el-button>
				</template>
			</el-table-column>
			<el-table-column prop="publisher_nickname" label="完成人昵称" width="150"></el-table-column>
			...
			<el-table-column prop="status" label="申请时间" width="180" sortable sort-by="applyTime">
				<template
					slot-scope="scope"
				>{{moment(scope.row.report_time * 1000).format("YYYY-MM-DD HH:mm:ss")}}</template>
			</el-table-column>
      ...
      layout="total, prev, pager, next, jumper" :total="total">
		...
		<el-dialog title="查看举报信息" :visible.sync="showAppealInfo">
			<el-form ref="form">
				<el-form-item label="用户昵称">
					<div class="txt-right">{{currAppealInfo.reporter_nickname}}</div>
				</el-form-item>
				<el-form-item label="举报时间">
					<div
						class="txt-right"
					>{{moment(currAppealInfo.report_time * 1000).format('YYYY-MM-DD HH:mm:ss')}}</div>
				</el-form-item>
				<el-form-item label="用户手机号">
					<div class="txt-right">{{currAppealInfo.reporter_phone}}</div>
				</el-form-item>
				<el-form-item label="举报原因">
					<el-input type="textarea" disabled :value="currAppealInfo.report_content"></el-input>
				</el-form-item>
			</el-form>
		</el-dialog>
	</div>
...
export default {
	...
	methods: {
		// 查询列表
		async queryList() {
			let res = await this.$api.getRepostList();
			if (res.code == 200) {
		...
		async showInfoClick(id) {
			let res = await this.$api.checkOperateInfo({ id: id });
			if (res.code == 200) {
				this.showInfo = true;
		...
		async verify(id) {
			...
			this.$prompt("", "填写处理备注", {
				...
				if (!val.value) {
					...
					let res = await this.$api.dealReport({
						id: id,
						reason: val.value
					});
					if (res.code == 200) {
